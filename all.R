			...
			a=1)						# 分散比
{
	Deming0 <- function(x, y)					# 1 組のデータについて，切片と傾きの推定値を計算する
	{
		...
		return(c(Intercept=mean(y)-Slope*mean(x), Slope=Slope))
	}
...
version2 <- function() cat("2015/12/18 (13:46:03)")
# 測定値をワイブル確率紙にプロットする
			...
			color="gray")			# 格子線を描く色
{
	...
	y <- weib(1:n/n)				# ワイブル分布における累積密度の位置
...
xtable.glm <- function(obj, caption="caption", label="label", vif=FALSE, align="lrrrr", digits=rep(3, 4), rev=-1.5, booktabs=FALSE, type=c("latex", "html")) {
...
xtable.lm <- function(obj, caption="caption", label="label", vif=FALSE, align="lrrrrrr", digits=rep(3, 6), footnote=rep(TRUE, 4), rev=-1.5, booktabs=FALSE, type=c("latex", "html")) {
